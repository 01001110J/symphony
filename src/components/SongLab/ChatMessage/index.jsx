...
const ChatMessage = ({ name, time, message, messageAsPre = false, isUserMessage = true }) => {
  return (
    <div className={`flex items-start gap-2.5 mb-5 ${isUserMessage ? 'flex-row-reverse' : ''}`}>
      ...
      >
        ...
        {messageAsPre ? (
          <pre className="text-sm font-normal py-2.5 text-gray-900 dark:text-white">{message}</pre>
        ) : (
